...
test("สลับภาษาเป็นอังกฤษและกลับมาไทยได้", async ({ page }) => {
  ...
  // หมวด/กล่องที่ seed มาเป็นไทยต้องแสดงเป็นอังกฤษด้วย (ไม่ใช่แค่ป้าย UI)
  ...
  await expect(page.getByText("อาหาร", { exact: true })).toHaveCount(0);
  // สลับกลับเป็นไทย — ชื่อ default ต้องกลับเป็นไทย
